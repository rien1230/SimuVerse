...
def _locked_call(entry, fn, *args, **kwargs):
    """
    Helper function to run a method under a run's lock. Ensuring thread safety when multiple
    clients try to interact with the same simulation run at the same time.
    """
    with entry.lock:
...
async def create_run(body: CreateRunBody) -> Dict[str, Any]:
    ...
    try:
        ...
        run_id = await anyio.to_thread.run_sync(registry.create_run, body.seed, body.config)
        entry = registry.get_run(run_id)
        return {"run_id": run_id, "status": entry.manager.status if entry else "created"}
...
async def start_run(run_id: str) -> Dict[str, Any]:
    """
    Start or resume a simulation run.
    Changes run status from 'idle' or 'paused' to 'running'.
    """
    entry = registry.get_run(run_id)
    if not entry:
...
async def pause_run(run_id: str) -> Dict[str, Any]:
    ...
    Changes run status from 'running' to 'paused'.

    """
    entry = registry.get_run(run_id)
    if not entry:
        # If run_id doesn't exist
        raise HTTPException(status_code=404, detail="Run not found")
...
async def stop_run(run_id: str) -> Dict[str, Any]:
    """
    Stop a simulation (terminal state).
    Changes run status to 'stopped'. Once stopped, a run cannot be resumed.
    """
    entry = registry.get_run(run_id)
    if not entry:
        # If run_id doesn't exist
...
async def step_run(run_id: str) -> Dict[str, Any]:
    """
    Execute one simulation tick (advance time by one step).
    This is the core simulation operation. It:
    1. Advances the simulation by one tick
    2. Returns the state changes (diff) that happened
    3. Updates the run status if the episode ended
    """
    entry = registry.get_run(run_id)
    if not entry:
        raise HTTPException(status_code=404, detail="Run not found")
...
async def delete_run(run_id: str) -> Dict[str, Any]:
    ...
    doesn't delete the replay file from disk, just removes
    the run from the active registry. The replay file remains for analysis.
    """
...
async def run_ws(websocket: WebSocket, run_id: str) -> None:
    ...
    - Get live updates as the simulation runs
    - Control the simulation (start, pause, step, auto-run)
    - See state changes immediately without polling
    """
    ...
    async def auto_loop(tick_hz: float) -> None:
        ...
        Used for the "auto" mode where the simulation runs by itself.
        """
    ...
    try:
        ...
        if auto_task and not auto_task.done():
            auto_task.cancel()  # Cancel auto task
...
def _inc_clients(run_id: str, delta: int) -> None:
    ...
    This tracks how many WebSocket connections are watching a run.
    Used for monitoring and potentially for cleanup decisions.
